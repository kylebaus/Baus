...
from ...core import Exchange, Account, OrderbookLevel, Side, OrderType, Instrument
...
#     {'sink': 'logs/binanceusdm_gateway.log', 'level': 'DEBUG', 'backtrace': False, 'diagnose': True,
#      'rotation': '00:00'}])


class BinanceusdmGateway(AbstractGateway):
    def __init__(self, executor, callbacks, config):
        super().__init__(Exchange.BINANCEUSDM, executor, callbacks)
        self.config = config
        self.http_client = HttpClient(self.executor, "https://fapi.binance.com", 10)
        ...
        self.account = Account(Exchange.BINANCEUSDM, config["account"]["name"])
        self.active = False
        ...
        self.ws_id = 0
        self.logger = logger
        self.logger.add(sink='logs/binanceusdm_gateway.log', level='DEBUG', backtrace=False,
    ...
    async def keep_alive_listen_key(self):
        while True:
            message = await self.http_client.put(endpoint='fapi/v1/listenKey',
                                                 header=self.get_headers('PUT'),
                                                 data=None)
            if message == {}:
                self.logger.info("BinanceusdmGateway: keep_alive_listen_key success")
            else:
                self.logger.info("BinanceusdmGateway: keep_alive_listen_key failure")
            await asyncio.sleep(60*30)

    def subscribe_fills(self):
        self.logger.info("BinanceusdmGateway: subscribe_fills")
    ...
    async def place_wrapper(self, order_id, order):
        try:
            ...
            # print(f'trying to place {order.price}')
            ...
            if 'orderId' in message:
                ...
                self.logger.info(f'BinanceusdmGateway: unable to parse place_wrapper {order.instrument.get_external_symbol()} {order.price} {order.quantity} message {message}')
                self.callbacks.on_order_update(PlaceReject(order_id, message))
                if "code" in message.keys():
                    if message["code"] == -1015 or message["code"] == -1001:  # 1015 is rate limit, 1001 is disconnect (which I think is related) TODO
                        self.rate_limited = True
                        self.rate_limited_ts = time.time()

        except Exception as e:
            self.logger.info(f'BinanceusdmGateway: place wrapper error {e}')
            # self.logger.info("BinanceusdmGateway: place_wrapper", order.instrument.get_external_symbol(), "Error", str(e))
            # self.logger.exception("BinanceusdmGateway: place_wrapper", order.instrument.get_external_symbol(), "Error", str(e))
    ...
    async def cancel_wrapper(self, order_id, instrument):
        try:
            ...
            self.logger.info((f"BinanceusdmGateway: cancel_wrapper error for {order_id}, reason: {e}"))
            # self.logger.exception((f"BinanceusdmGateway: cancel_wrapper error for {order_id}, reason: {e}"))
    ...
    def subscribe_orderbook(self, instrument: Instrument):
        self.logger.info("BinanceusdmGateway: subscribe_orderbook")
    ...
    def get_funding(self, account, instrument: Instrument):
        ...
        payload = {'symbol': instrument.get_external_symbol()}
        signed_payload_str = self.sign_payload(payload)
        task = self.executor.create_task(self.http_client.get(endpoint=f"{endpoint}?{signed_payload_str}",
                                                              header=self.get_headers('GET')))
        self.tasks.add(task)
    ...
    def parse_websocket(self, message):
        try:
            ...
            if "e" in message.keys():
                if message["e"] == "bookTicker":
                    ...
                    if message["o"]["L"] != "0":  #TODO confirm this is the way to filter for fills
                        ...
                        # print("BinanceusdmGateway: parse_websocket: fill msg", ext_id, message)
                        if ext_id in self.external_to_internal_map.keys():
                            order_id = self.external_to_internal_map[ext_id]
                        else:
                            order_id = None
                            # self.logger.info(f"BinanceusdmGateway: parse_websocket: Fill internal id not in map, "
                    ...
                    pass
                    # print("BinanceusdmGateway: parse_websocket: event type not parsed", message)
            else:
                pass
                # print("BinanceusdmGateway: parse_websocket: no event in message keys", message)
            ...
            self.logger.info("BinanceusdmGateway: parse_websocket exception: ", str(e))
            # self.logger.exception("BinanceusdmGateway: parse_websocket exception: ", str(e))

    def notify_disconnect(self):
        self.logger.info("BinanceusdmGateway: notify_disconnect")
        self.callbacks.on_gateway_update(GatewayDisconnect(self.account))
        self.reconnect()
        
    def reconnect(self):
        self.logger.info("BinanceusdmGateway: reconnect")
        self.websocket_client = WebsocketClient(self.executor, self.config["host"], 
                                                self.parse_websocket, self.notify_disconnect)
        self.run()
                
    def run(self):
        self.logger.info("BinanceusdmGateway: run")
        status = asyncio.Event()
        task = self.executor.create_task(self.websocket_client.run(status))
        self.tasks.add(task)

        self.logger.info("BinanceusdmGateway: keep_alive_listen_key")
